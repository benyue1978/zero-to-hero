...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # remove after using torch's built in flash attention, which does not require an explicit mask buffer
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            ...
            if hasattr(module, 'NANOGPT_SCALE_INIT'):
                std *= (2 * self.config.n_layer) ** -0.5 # for residual connections, per https://arxiv.org/abs/2104.07857
...
class ShakespearDataLoaderLite:
    ...
    def __init__(self, B, T, process_rank, num_processes):
        ...
        # state
        self.current_position = self.B * self.T * self.process_rank # each process starts at a different offset into the data, to ensure we cover the whole dataset evenly across processes
    ...
    def next_batch(self):
        ...
        x = buf[:-1].view(B, T)
        y = buf[1:].view(B, T)
        # advance the current position and wrap around if necessary
        self.current_position += B * T * self.num_processes
        # if loading the next batch would go past the end of the dataset, wrap around to the beginning
...
class DataLoaderLite:
    ...
    def __init__(self, B, T, process_rank, num_processes, split):
        ...
        shards = sorted(shards) # ensure the shards are always in the same order
    ...
    def next_batch(self):
        ...
        # if loading the next batch would go past the end of the dataset, wrap around to the beginning
        if self.current_position + B * T * self.num_processes + 1 >= len(self.tokens):
            self.current_shard = (self.current_shard + 1) % len(self.shards) # move to the next shard, wrap around to zero if at the end
...
# DDP launch for e.g. 8 GPUs: # https://docs.pytorch.org/docs/main/generated/torch.nn.parallel.DistributedDataParallel.html
...
if ddp:
    ...
    # non-DDP mode, just run on whatever device is available
    ddp_rank = 0
    ddp_local_rank = 0
    ddp_world_size = 1
    master_process = True
...
total_batch_size = 524288 # 2**29 ~0.5M, in number of tokens
B = 64 # micro batch size, in number of sequences
T = 1024 # sequence length, in number of tokens
# testing
# total_batch_size = 131072 # for testing
# B = 4 # micro batch size, in number of sequences
# T = 1024 # sequence length, in number of tokens

assert total_batch_size % (B * T * ddp_world_size) == 0, "total_batch_size must be divisible by B * T * ddp_world_size"
grad_accum_steps = total_batch_size // (B * T * ddp_world_size) # number of micro steps to accumulate before each optimizer step
if master_process:
    print(f"total batch size: {total_batch_size}")
    print(f"calculated gradient accumulation steps: {grad_accum_steps}")

# print(f"I'm running on GPU {ddp_local_rank}")
# destroy_process_group()
# import sys; sys.exit()

train_loader = DataLoaderLite(B=B, T=T, process_rank=ddp_local_rank, num_processes=ddp_world_size, split='train') # adjust B to fit in memory, adjust T for context length (try 512 or 1024)
val_loader = DataLoaderLite(B=B, T=T, process_rank=ddp_local_rank, num_processes=ddp_world_size, split='val')   
# testing
# train_loader = ShakespearDataLoaderLite(B=B, T=T, process_rank=ddp_local_rank, num_processes=ddp_world_size)
...
warmup_steps = 715
max_steps = 19073
# # testing, uncomment these lines on the dev machine to make sure the code runs
# warmup_steps = 10
# max_steps = 50
...
for step in range(max_steps):
    ...
    # once in a while evaluate the val loss to see how we're doing
    if step % 250 == 0 or last_step:
        model.eval()
        val_loader.reset() # start from the beginning of the validation set
        with torch.no_grad():
            val_loss_accum = 0.0
            val_loss_steps = 20
        ...
        if ddp:
            dist.all_reduce(val_loss_accum, op=dist.ReduceOp.AVG) # average the loss across processes for logging
        if master_process:
    ...
    t1 = time.time()
    dt = t1 - t0
    token_processed = grad_accum_steps * train_loader.B * train_loader.T * ddp_world_size
